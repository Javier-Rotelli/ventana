    ...
    for (const landmarks of results.faceLandmarks) {
      ...
      midX =
        Math.round(
          100 *
            (video.videoWidth / 2 -
              (video.videoWidth * (landmarks[468].x + landmarks[473].x)) / 2)
        ) / 100;
      midY =
        Math.round(
          100 *
            ((video.videoHeight * (landmarks[468].y + landmarks[473].y)) / 2 -
              video.videoHeight / 2)
        ) / 100;

      //   viewpoint.style.perspectiveOrigin = `${50 + midX * persFactX}% ${
      //     50 + midY * persFactY
      //   }%`;
      //   canvas.style.perspectiveOrigin = `${50 + midX * persFactX}% ${
      //     50 + midY * persFactY
      //   }%`;
      //   return [midX, midY];
